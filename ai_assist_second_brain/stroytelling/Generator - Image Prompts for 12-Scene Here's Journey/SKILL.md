---
name: Generator - Image Prompts for 12-Scene Here's Journey
description: Генерирует 12-сценный сториборд «Путешествие героя», где каждая сцена — самостоятельный промпт для генерации изображения: никакая информация не переносится между сценами, поэтому каждая отрисовывается независимо.
---
